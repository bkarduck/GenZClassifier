...
from redditmodel import word_ngrams
...
class CrazyTokenizer(object):
    ...
    def __init__(self, lowercase=True, keepcaps=False, normalize=3,
        ...
        #self.nlp = self.nlp
        self._merging_matcher = Matcher(self.nlp.vocab)
        ...
        alpha_digits_flag = self.nlp.vocab.add_flag(alpha_digits_check)
        ...
        if reddit_usernames is not False:
            ...
            reddit_username_flag = self.nlp.vocab.add_flag(
                reddit_username_check)
            reddit_username_flag2 = self.nlp.vocab.add_flag(
        ...
        if subreddits is not False:
            ...
            subreddit_flag2 = self.nlp.vocab.add_flag(subreddit_check2)
            #subFlagID1 = self.nlp.vocab.check_flag(subreddit_flag)
        ...
        if isinstance(extra_patterns, list):
            ...
            for name, re_pattern, replacement_token in extra_patterns:
                ...
                self._flags[name] = self.nlp.vocab.add_flag(flag)
        ...
        retokenize_flag = self.nlp.vocab.add_flag(retokenize_check)
    ...
    def tokenize(self, text):
        ...
        #doc1 = self.nlp.make_doc(text)  # Create a Doc from raw text
       
            #doc1 = self.proc(doc=doc1)  
        #self.nlp = self.nlp(text)
        #doc1 = self.nlp(doc1)
        tokens = doc._.tokens
